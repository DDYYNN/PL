...
struct
  ...
  let op2fn =
    ...
    | EQ ->  (fun (v1,v2) -> 
                    ...
                    |(String s1, String s2) -> if ((String.compare s1 s2)=0) then
                            Bool true else Bool false
                    |(Pair _, _) -> raise (TypeError "Pair in EQ")
                    |(Closure _, _) -> raise (TypeError "Closure in EQ")
                    | _ -> raise (TypeError "EQ error"))
