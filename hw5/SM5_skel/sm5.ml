...
struct
  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
      ...
      let rec memory_mark loc =
              reachable_locs := loc :: !reachable_locs;
              match (load loc m) with
              | L loc' -> memory_mark loc'
              | R record -> List.iter (fun (v, loc') -> memory_mark loc') record
              | _ -> () in
      let rec env_mark (_, evalue) =
              match evalue with
              | Loc loc -> memory_mark loc
              | Proc (_, _, env') -> List.iter env_mark env' in
      let stack_mark svalue =
              match svalue with
              | V value ->
                              (match value with
                              | L loc -> memory_mark loc
                              | R record -> List.iter (fun (v,loc') -> memory_mark loc') record
                              | _ -> ())
              | P (_, _, env) -> List.iter env_mark env
              | M (id, evalue) -> env_mark (id, evalue) in
      let _ = List.iter (fun svalue -> stack_mark svalue) s in
      let _ = List.iter (fun x -> env_mark x) e in
      let _ = List.iter (fun (_, env) -> List.iter env_mark env) k in
